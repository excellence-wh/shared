...
async function executeRestore(
  ...
  if (project.type === "node") {
    ...
    // 优先使用pnpm，其次使用npm，并添加--verbose参数
    command = isPnpmAvailable
...
async function main() {
  ...
  async function processQueue() {
    while (projects.length > 0) {
      ...
      console.log(
        `🔄 [${processed}/${projects.length + processed}] 处理 ${project.type.toUpperCase()} 项目: ${project.path}`,
      );

      const result = await executeRestore(project, dryRun, verbose);
      results.push({
        project,
  ...
  // 生成最终报告
  console.log(`📊 处理完成 ${results.length} 个项目:`);
